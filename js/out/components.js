...
function bound01(n8, max2) {
  if (isOnePointZero(n8))
    n8 = "100%";
  var processPercent = isPercentage(n8);
  n8 = Math.min(max2, Math.max(0, parseFloat(n8)));
  if (processPercent) {
    n8 = parseInt(n8 * max2, 10) / 100;
  }
  if (Math.abs(n8 - max2) < 1e-6) {
    return 1;
  }
  return n8 % max2 / parseFloat(max2);
}
...
var ImmutableSet = class _ImmutableSet {
  ...
  static empty() {
    return new _ImmutableSet(/* @__PURE__ */ new Set());
  }
  has(value) {
...
__decorateClass([
  n6({ type: Boolean })
], ShinyCard.prototype, "shadowed", 2);
__decorateClass([
...
var ShinyDashboard = class extends s4 {
  ...
  watchMainSlot(e9) {
    this.tabs = getElementsFromSlotChangeEvent(e9).reduce(
      (all, node) => {
  ...
  selectTab(tabIndex = this.selected_tab_index) {
    this.selected_tab_index = tabIndex;
    this.tabs.forEach((tab, i6) => {
      const isSelected = i6 === tabIndex;
  ...
  showHeader() {
    if (this.hasHeaderContents)
      return true;
    if (this.tabs.length > 0 && !this.sidebarNavigation)
      return true;
    return false;
  }
  render() {
    const tabs = this.tabs.map((tab, i6) => {
      const isSelected = i6 === this.selected_tab_index;
      return x`<div
    ...
    const tabContainer = x`<div class="nav">
...
__decorateClass([
  n6({ type: Number })
], ShinyDashboard.prototype, "selected_tab_index", 2);
__decorateClass([
  n6({ type: Boolean })
], ShinyDashboard.prototype, "sidebarNavigation", 2);
__decorateClass([
  t5()
...
var Sidebar = class extends s4 {
  ...
  handleSlottedElementsChange(e9) {
